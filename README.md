...
#git remote로 연결된 리모트 저장소 확인
# git remote show "[remote name]"으로 연결된 리모트 저장소의 구체적인 정보 확인
# git remote rename "[Before]" "[After]"로 로컬에 저장된 리모트 저장소 이름 변경
# git remote rm "[remote name]"으로 로컬에서 리모트 저장소 포인터 삭제
...
#git fetch로 리모트 브랜치(리모트 트래킹 브랜치)들을 내려받은 뒤, Merge하지 않고 아예 해당 브랜치의 커밋에 해당하는 새 브런치를 만들려면
...
#과는 다르게 아예 리모트 서버에 있는 리모트 브랜치를 제거해버린다.
